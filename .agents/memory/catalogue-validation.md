...
- Character enrichment: pins.main_character / all_characters (semicolon list) cover the whole catalogue but sometimes hold venues or repeat the title — always run entries through the looksLikeCharacter guard (in catalogue-validation service) before using them as search terms or scoring constraints.
