...
All migrations through 014 applied as of 2026-07-25. Realtime: tables must be added to the `supabase_realtime` publication to emit events; it includes `trade_messages`, `trades`, `user_pins`, `pin_submissions`, `community_posts`, and `post_comments` (migration 014). RLS filters delivered events, so subscribers only receive rows their select policies allow.
