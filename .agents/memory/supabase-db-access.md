...
Realtime: tables must be added to the `supabase_realtime` publication to emit events; RLS filters delivered events, so subscribers only receive rows their select policies allow.
