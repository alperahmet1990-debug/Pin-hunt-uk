  ...
  return (
    <main className="pha-shell">
      <style>{`
        ...
        .pha-find{width:100%;min-height:139px;position:relative;overflow:hidden;padding:18px;text-align:left;border-radius:25px;background:var(--plum);color:#fff5e8;box-shadow:0 13px 28px #4b214648;transition:transform .2s ease,box-shadow .2s ease}.pha-find:hover{transform:translateY(-2px);box-shadow:0 17px 30px #4b214658}.pha-find:before{content:"";position:absolute;width:170px;height:170px;border-radius:50%;border:25px solid #f4ca6940;right:-61px;bottom:-78px}.pha-find:after{content:"";position:absolute;width:82px;height:82px;border-radius:22px;border:1px solid #f9dca477;transform:rotate(25deg);right:29px;top:-47px}.pha-findcopy{position:relative;z-index:1;width:190px}.pha-findlabel{display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#f7cfa2}.pha-find h1{font-size:27px;line-height:28px;margin:8px 0 5px;font-weight:850;letter-spacing:-.05em}.pha-find p{font-size:13px;margin:0;color:#f7dfca;line-height:17px;font-weight:500}.pha-findicon{position:absolute;right:22px;bottom:20px;z-index:1;width:48px;height:48px;border-radius:17px;background:var(--butter);display:grid;place-items:center;color:var(--plum);box-shadow:0 7px 13px #26132950}
      ...
      <div className="pha-scroll">
        ...
        <button type="button" onClick={() => setFound(!found)} className="pha-find" aria-label="Find a pin"><span className="pha-findcopy"><span className="pha-findlabel"><Sparkles size={13}/> {found ? "Ready when you are" : "Your pin detective"}</span><h1>Find a Pin</h1><p>Scan or search the catalogue</p></span><span className="pha-findicon">{found ? <Search size={23}/> : <Camera size={23}/>}</span></button>
        ...
        <section className="pha-section"><div className="pha-sectionhead"><h2>For You</h2><button type="button" onClick={noop}>See all</button></div><button type="button" onClick={noop} className="pha-update"><span className="pha-update-icon"><Sparkles size={18}/></span><span className="pha-update-text"><b>Your submission is live</b><span>Festival of Fantasy Mickey was approved</span></span><ChevronRight size={17}/></button></section>
