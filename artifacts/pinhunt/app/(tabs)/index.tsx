...
import { AccessibilityInfo, Animated, Easing, Linking, Platform, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
import { communityPostToEvent, selectUpcomingEvents } from '@/utils/events';
...
export default function HomeScreen() {
  ...
  const pickupPostById = useMemo(() => new Map(pickupPosts.map(post => [`community-${post.id}`, post])), [pickupPosts]);
  const whatsHappening = useMemo(
    () => selectUpcomingEvents([...EVENTS, ...pickupPosts.map(communityPostToEvent)]),
    [pickupPosts],
  );
  const handleViewEvent = (event: PinHuntEvent) => {
    if (event.sourceType === 'community') {
      const post = pickupPostById.get(event.id);
  ...
  return <View style={[styles.root, { backgroundColor: colors.homeBackground }]}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { paddingTop: topPad + 12, paddingBottom: bottomPad }]}>
      ...
      {whatsHappening.length > 0 && <Section title="What’s Happening" subtitle="Pin drops, trading events & community meet-ups" colors={colors} actionLabel="See all" action={() => router.push({ pathname: '/(tabs)/community', params: { filter: 'new_pickup' } })}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.eventsShelf}>
          {whatsHappening.map(event => <EventCard key={event.id} event={event} onPress={() => handleViewEvent(event)} />)}
        </ScrollView>
      </Section>}
